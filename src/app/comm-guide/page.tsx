...
这些不是刁难，是正常的项目保障。你们调整一下我们再谈。」`,
  ...
  // ── 甲方实战进阶：更真实的复杂场景 ──
  {
    ...
    title: "供应商中途换项目经理",
    scenario: "objection",
    content: `「等一下，你说换项目经理？这个事你得提前跟我沟通，不是通知我。

我关心的是：
1. 新的项目经理了解我们项目吗？什么时候能完全接手？
2. 之前的承诺和约定，新PM认不认？
3. 过渡期间会不会影响开发进度？

说实话，我们选你们 partly 因为之前那个PM的能力。换人可以，但我需要看到新PM的资历和交接计划。如果因为换人导致项目延期，这个责任怎么算？

建议这周安排一次三方会议，你、老PM、新PM一起，把所有约定过的事项重新确认一遍。」`,
    tips: "换人是大事，不能接受口头通知。要求正式交接会，白纸黑字确认所有历史约定。",
  },
  {
    id: "a-adv-2",
    title: "供应商拿别的医院数据忽悠",
    scenario: "rejection",
    content: `「你说XX医院也在用你们系统，效果很好？我有几个问题：

1. 他们用的是标准版还是定制版？跟我们一样吗？
2. 他们的业务场景跟我们一样吗？他们XX科室和我们的流程差异很大
3. 那个实施案例的数据我可以去核实吗？

我之前就遇到过这种情况——供应商拿一个三甲医院的案例来说事，结果人家是定制版花了额外200万，标准版根本不是那么回事。

这样吧，你能安排我去那家医院实地看看吗？或者给我一个他们信息科的联系人和电话，我自己去了解。如果真像你说的，这反而是加分项。」`,
    tips: "别被'XX医院也在用'唬住。追问版本、场景差异，要求核实。能安排实地参观更好。",
  },
  {
    id: "a-adv-3",
    title: "验收发现功能和合同不一致",
    scenario: "objection",
    content: `「我直说吧，这次验收我们没过。

我拿着合同附件的需求清单逐条核的，有以下问题：
1. XX功能——合同写的是'支持XX流程自动流转'，实际只做了手动触发
2. XX报表——合同写的'支持自定义报表'，现在只能用预设的5个模板
3. 数据导入——合同写的'支持Excel批量导入'，结果超过1000条就报错

这些不是小问题。我们的立场是：
- 差异项必须在本月内修复
- 如果确实做不到，需要给出替代方案和补偿措施
- 修复完成前，对应模块的验收款我们不会付

你回去和团队对一下，给我一个书面回复。我们的法务也在看这个事。」`,
    tips: "验收要拿合同逐条对，不能凭感觉。发现问题要书面记录、限期修复、 withhold付款。提法务是给压力但不用真打官司。",
  },
  {
    id: "a-adv-4",
    title: "供应商忽悠说\"做不到\"",
    scenario: "rejection",
    content: `「你说这个功能做不到？我需要理解一下原因：

1. 是技术上做不到，还是你们产品架构不支持？
2. 如果是架构问题，是永远做不了还是需要时间改造？
3. 你们有没有类似的替代方案？

我为什么追问这个——因为我们去XX医院参观的时候，看到他们用的XX系统就有这个功能。而且那个系统还不是特别大的厂商。

我的判断是，不是做不到，是你们不想花这个精力。如果真是这样，我们可以重新评估一下合作范围。

要不这样，你回去跟你们技术总监确认一下，是真的做不到还是成本太高。如果是成本问题，咱们可以谈。但别用'做不到'三个字就打发我。」`,
    tips: "'做不到'分两种：真做不到和不想做。用竞品案例反驳，给台阶说是成本问题可以谈。让对方知道你不是好忽悠的。",
  },
  {
    id: "a-adv-5",
    title: "供应商画大饼但迟迟不签合同",
    scenario: "progress",
    content: `「张总，咱们聊了三个月了，方案改了四版，演示做了两轮，但合同一直没签。

我这边的情况是这样的：
1. 院领导对这个项目已经很关注了，问了多次进展
2. 我们的信息化预算是年度的，再不签合同，这笔钱可能要被收回去
3. 其他几个供应商一直在联系我们

我的建议是——这周之内把合同定下来。有几个条款我们各退一步：
- 付款可以按你们说的分四期，但每期要绑定具体交付物
- 运维费我们可以接受你们的标准，但SLA要写具体数字

如果不方便签，你告诉我卡在哪里了，我看看能不能帮你推动。但不能再拖了，下周三之前要有一个结果。」`,
    tips: "给deadline但给台阶。透露有竞品在接触给紧迫感。主动问卡在哪里体现合作态度。关键是设定明确的截止日期。",
  },
  {
    id: "a-adv-6",
    title: "供应商偷换概念降低交付标准",
    scenario: "rejection",
    content: `「等等，我注意到一个问题。上次需求确认会我们说好的是XX，你现在交付的是YY，这两个不是一个东西。

你当时说的是'支持全院级的数据分析'，现在交付的是'单个科室的报表查询'。这个差距太大了。

我不想争论当时说的是什么，因为我们有会议纪要。我现在把当时的纪要发给你，你自己对照一下。

我的态度很明确：按原定标准交付。如果你们觉得确实有困难，我们坐下来谈怎么调整，但调整的前提是——你要先承认差距，不是悄悄降标准然后假装就是这样的。」`,
    tips: "保留所有会议纪要和沟通记录。发现偷换概念立刻指出，拿证据说话。给对方台阶但要求先承认问题。",
  },
  {
    id: "a-adv-7",
    title: "供应商的售后响应太慢",
    scenario: "progress",
    content: `「王总，我这边统计了一下最近三个月的售后工单：

- 一共提了27个工单
- 平均响应时间3.2天（合同写的是4小时）
- 其中5个到现在还没解决，最长的已经45天了
- 有3次系统宕机，每次恢复都超过4小时

这个数据我不展开说了。我的要求是：
1. 未解决的5个工单，这周内必须给解决方案
2. 以后工单响应必须在合同约定时间内
3. 如果再次出现宕机超时，按合同违约条款处理

我建议我们每月做一次运维Review，把数据摆在桌面上。我不希望走到扣款那一步，但数据在这里，你们看着办。」`,
    tips: "用数据说话——统计工单数、响应时间、未解决数。提合同条款给压力但保持合作基调。建议制度化Review。",
  },
  {
    id: "a-adv-8",
    title: "供应商要涨价（二期或续约）",
    scenario: "objection",
    content: `「涨价？你们得给我一个合理的解释。

1. 一期的价格是XX，二期你说涨30%，依据是什么？
2. 是开发成本增加了，还是你们调价了？
3. 如果是人天单价涨了，那我们看下二期实际需要多少人天

我也跟你说实话——我这边也做了功课。同类产品二期的价格一般是一期的40-60%。你们现在报的这个数，已经超出了我的预期。
...
你觉得这个方案能接受吗？」`,
    tips: "要求涨价依据，用市场行情锚定价格。给出你的预算范围，用后续合作做筹码。谈判要给对方台阶。",
  },
];
...
2. **要求实地验证**：
   - 「能安排我去那家医院看看吗？」
   - 「能给我他们信息科的联系方式吗？我想直接了解一下」

3. **如果供应商拒绝**：
   - 拒绝安排参观 → 案例大概率有问题
   - 说有保密协议 → 合理，但可以要求供应商提供脱敏数据
   - 推三阻四 → 直接降分
...
4. **自己的功课**：
   ...
   - 在行业群里问一圈
   - 要求看系统截图而非PPT`,
    followUp: "核心原则：PPT谁都能做，真实案例需要验证。能实地看就不要只听描述。",
  },
  {
    id: "a-faq-7",
    question: "\"项目上线后供应商就不管了怎么办？\"",
    answer: `**预防和应对：**

1. **合同阶段就要约定**：
   - 明确运维服务范围和SLA（如：4小时响应、24小时解决）
   - 约定运维费用和付款方式
   - 写清楚违约责任
...
4. **长期保障**：
   ...
   - 关键系统的管理员权限要在自己手里
   - 和第二供应商保持联系，作为备选`,
    followUp: "核心原则：运维条款在签合同时就要谈好。上线后的问题用数据和合同说话。",
  },
  {
    id: "a-faq-8",
    question: "\"供应商中途换人（PM、开发团队）怎么办？\"",
    answer: `**应对策略：**
...
4. **写在合同里（以后的项目）**：
   - 关键人员变更需甲方书面同意
   - 如未经同意换人，甲方有权终止合同
   - 定义哪些是'关键人员'（项目经理、技术负责人等）`,
    followUp: "核心原则：换人不能只是通知，必须走正式交接。以后的项目要把人员变更条款写进合同。",
  },
];
