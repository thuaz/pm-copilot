...
const CATEGORIES: CategoryMeta[] = [
  { key: "opening", label: "开场白", icon: MessageCircle, color: "text-blue-600 bg-blue-50" },
  { key: "requirement", label: "需求确认", icon: Lightbulb, color: "text-amber-600 bg-amber-50" },
  { key: "rejection", label: "拒绝/拖延", icon: MessageSquare, color: "text-red-600 bg-red-50" },
  { key: "progress", label: "汇报进展", icon: BookOpen, color: "text-green-600 bg-green-50" },
  { key: "objection", label: "处理异议", icon: HelpCircle, color: "text-purple-600 bg-purple-50" },
  { key: "faq", label: "甲方 FAQ", icon: HelpCircle, color: "text-indigo-600 bg-indigo-50" },
];

// ── Preset data: communication scripts ──
...
export default function CommGuidePage() {
  ...
  const filteredScripts = useMemo(() => {
    ...
    if (activeCategory !== "faq") {
      items = SCRIPTS.filter((s) => s.scenario === activeCategory);
    }
    if (searchQuery.trim()) {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold">甲方沟通教练</h1>
        <p className="text-[var(--color-muted-foreground)] mt-1">
          PM 和甲方沟通的参考话术库，包含各类场景的应对模板和行业 FAQ
        </p>
      </div>
